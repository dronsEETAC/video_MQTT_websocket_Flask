...
def index():
    ...
    return render_template('indexSoloSockets.html')
...
# aquí tenemos las diferentes peticiones del navegador que llegan por websocket y
# retrasmito a la estación de tierra también por el websocket
@socketio.on("conectar")
def handle_conectar():
    socketio.emit("conectar")

@socketio.on("arm_takeOff")
def handle_arm_takeOff(altura):
    socketio.emit("arm_takeOff", altura)
...
@socketio.on("stopRecord")
def handle_stopRecord():
...
@socketio.on("telemetryInfo")
def handle_telemetryInfo(info):
    socketio.emit("telemetryInfo", info)
